...
namespace Asistant_Domain_Core.RequestAgg.Services
{
    public interface IRequestService
    {
        ...
        Task<bool> ChangeRequestToPendingSuggestionApproval(int requestId, CancellationToken ct);
        ...
        Task<PagedResult<OutputRequestDTO>> GetPagedRequestForExpert(int cityId, List<int> homeServicesId, int pageNumber, int pageSize, CancellationToken ct);
        Task<OutputRequestDTO?> GetRequestById(int id, CancellationToken ct);
        Task<bool> IsRequestForCustomer(int customerId, int requestId, CancellationToken ct);
        Task<bool> RejectRequest(int id, CancellationToken ct);
        Task<int> SuggestionCount(int requestId, CancellationToken ct);
